...
// ====== SÓ O NOME, SEM NENHUM AVATAR (LIMPO E PROFISSIONAL) ======
function createMessageElement(msg) {
    const div = document.createElement('div');
    const isSent = msg.senderId === currentUserId;
    div.className = `message ${isSent ? 'sent' : 'received'}`;

    const time = msg.timestamp?.toDate().toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'}) || '';

    // Só aparece o nome nas mensagens recebidas
    const nameHtml = !isSent ? `<div class="sender-name-only">${currentFriend.name}</div>` : '';

    div.innerHTML = `
        ${nameHtml}
        <div class="message-content">
            ${msg.text || ''}
            ${msg.mediaUrl ? `<img src="${msg.mediaUrl}" class="message-media" style="border-radius:8px;margin-top:5px;">` : ''}
